...
Após as [correções de dependências](SECURITY_DEPENDENCIES_2026_09.md), a homologação recebeu um endereço HTTPS temporário e passou novamente no teste HTTP integrado com dois clientes. Os links de login agora usam a origem definida em `NEXTAUTH_URL` de `.env.staging`; a captura de e-mails continua apenas em `http://localhost:8026`. A validação interativa em navegador e o teste real com a Meta permanecem pendentes. Ver também [implantação de homologação](STAGING_INSTAGRAM.md) e [análise do aplicativo](META_APP_REVIEW.md).
